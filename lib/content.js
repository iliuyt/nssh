...
let init = function () {
    try {
        ...
        // 判断是否有密钥
        fs.lstat(common.SSHPrivateKeyPath, function (err, stats) {
            if (!err && !stats.isSymbolicLink() && !fs.existsSync(common.DefaultPath)) {
...
let use = function (name) {
    ...
    fs.access(common.SSHPrivateKeyPath, function (err) {
        if (!err) {
            // 删除
            fs.unlinkSync(common.SSHPrivateKeyPath);
            fs.unlinkSync(common.SSHPublicKeyPath);
...
let rename = function (name, newName) {
    ...
    let keys = utils.loadSSHKeys(common.NSSHPath);
    if (keys[newName]) {
        console.log(chalk.red(newName + '密钥已存在'));
        return false;
    }
